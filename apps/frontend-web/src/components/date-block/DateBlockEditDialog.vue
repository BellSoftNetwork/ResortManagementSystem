<template>
  <q-dialog v-model="dialog.isOpen" :persistent="status.isProgress">
    <q-card style="width: 500px">
      <q-card-section class="text-h6">예약 마감 수정</q-card-section>
...
import { updateDateBlock } from "src/api/v1/date-block";
...
function open(dateBlock: DateBlock) {
  currentId.value = dateBlock.id;
  formData.value = {
    startDate: dateBlock.startDate,
    endDate: dateBlock.endDate,
...
function update() {
  ...
  updateDateBlock(currentId.value, formData.value)
    .then(() => {
      emit("updated");
