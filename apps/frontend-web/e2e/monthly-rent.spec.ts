...
test.describe('Monthly Rent CRUD Tests', () => {
  test.describe.configure({ mode: 'serial' });

  let createdMonthlyRentId: number | null = null;
  let testGuestName: string;

  test.beforeAll(async () => {
    testGuestName = `달방테스트-${generateUniqueId()}`;
  });

  test('달방 목록 조회', async ({ page }) => {
    // given: 사용자가 로그인된 상태
    await loginViaUI(page, TEST_USER.userId, TEST_USER.password);

    // when: 달방 목록 페이지 접근
    await page.goto('/#/monthly-rents');
    await page.waitForLoadState('networkidle');

    // then: 테이블이 표시됨
    await expect(page.locator('.q-table')).toBeVisible();
    await expect(page.locator('.q-table__title')).toContainText('숙박 중인 달방');

    // then: 상세 검색 버튼이 있음
    await expect(page.locator('button:has-text("상세 검색")').first()).toBeVisible();

    // then: 추가 버튼이 있음
    const addButton = page.locator('a[href*="monthly-rents/create"], button:has(i.q-icon:text("add"))').first();
    await expect(addButton).toBeVisible();
  });

  test('달방 생성', async ({ page }) => {
    // given: 사용자가 로그인된 상태
    await loginViaUI(page, TEST_USER.userId, TEST_USER.password);

    // when: 달방 생성 페이지 접근
    await page.goto('/#/monthly-rents/create');
    await page.waitForLoadState('networkidle');

    // then: 생성 폼이 표시됨
    await expect(page.locator('.q-card')).toBeVisible();
    await expect(page.getByText('달방 등록')).toBeVisible();
    ...
    // then: 목록 페이지로 리다이렉트됨
    expect(page.url()).toContain('/#/monthly-rents');
    expect(page.url()).not.toContain('/create');

    // then: 생성된 달방이 목록에 표시됨
    await expect(page.locator('.q-table')).toBeVisible();
    ...
    if (await rowWithName.isVisible()) {
      ...
      if (await detailLink.isVisible()) {
        ...
        const match = href?.match(/\/monthly-rents\/(\d+)/);
        if (match) {
          createdMonthlyRentId = parseInt(match[1], 10);
        }
      }
    }
  });

  test('달방 상세 조회', async ({ page }) => {
    test.skip(!createdMonthlyRentId, 'No monthly rent created in previous test');

    // given: 사용자가 로그인된 상태
    await loginViaUI(page, TEST_USER.userId, TEST_USER.password);

    // when: 달방 상세 페이지 접근
    await page.goto(`/#/monthly-rents/${createdMonthlyRentId}`);
    await page.waitForLoadState('networkidle');

    // then: 상세 정보가 표시됨
    await expect(page.locator('.q-card')).toBeVisible();
    await expect(page.getByText('달방 정보')).toBeVisible();
    
    // then: 투숙객명이 input에 표시됨
    const nameInput = page.locator('input[placeholder="홍길동"]');
  ...
  test('달방 수정', async ({ page }) => {
    test.skip(!createdMonthlyRentId, 'No monthly rent created in previous test');

    // given: 사용자가 로그인된 상태
    await loginViaUI(page, TEST_USER.userId, TEST_USER.password);

    // when: 달방 상세 페이지에서 수정 버튼 클릭
    await page.goto(`/#/monthly-rents/${createdMonthlyRentId}`);
    await page.waitForLoadState('networkidle');
    ...
    await expect(page.getByText('달방 수정')).toBeVisible();

    // when: 메모 수정
    const noteInput = page.locator('textarea');
    await noteInput.fill('E2E 달방 테스트 수정됨');
  ...
  test('달방 삭제 (상세 페이지에서)', async ({ page }) => {
    test.skip(!createdMonthlyRentId, 'No monthly rent created in previous test');
    ...
    // then: 목록 페이지로 리다이렉트됨
    expect(page.url()).toContain('/#/monthly-rents');
    expect(page.url()).not.toContain(`/${createdMonthlyRentId}`);

    // Mark as deleted to prevent double cleanup
    createdMonthlyRentId = null;
  });
});
