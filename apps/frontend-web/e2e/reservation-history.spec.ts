...
test.describe('Reservation History Tests', () => {
  ...
  test('예약 수정 후 변경 이력 테이블 자동 갱신', async ({ page }) => {
    ...
    // then: 상세 정보와 변경 이력 테이블이 표시됨
    await expect(page.locator('.q-card')).toBeVisible();
    await expect(page.getByText('변경 이력')).toBeVisible();

    // when: 초기 이력 항목 수 확인 (CREATED 이력 1개)
    await page.waitForTimeout(500);
    const historyTable = page.locator('.q-table').filter({ hasText: '변경 이력' });
    ...
    await noteInput.fill('E2E 테스트 - 이력 갱신 확인용 메모');
    ...
    // then: 페이지 새로고침 없이 이력 테이블이 자동 갱신됨
    // 이력 항목이 2개가 되어야 함 (CREATED + UPDATED)
    const historyRows = historyTable.locator('.q-table__middle tbody tr');
    const rowCount = await historyRows.count();

    expect(rowCount).toBeGreaterThanOrEqual(2);

    // then: 첫 번째 이력은 CREATED, 두 번째 이력은 UPDATED 타입이어야 함
    const firstRowType = historyRows.nth(0).locator('td').nth(0);
    await expect(firstRowType).toContainText('CREATED');

    const secondRowType = historyRows.nth(1).locator('td').nth(0);
    await expect(secondRowType).toContainText('UPDATED');
  });

  test('변경 내역에 메타 필드 미표시 확인', async ({ page }) => {
    test.skip(!createdReservationId, 'No reservation created in previous test');
    ...
    // when: 예약자명 수정
    const nameInput = page.locator('input[placeholder="홍길동"]');
    await nameInput.fill(`${testGuestName}-수정됨`);
    ...
    // when: 최신 이력 항목의 변경 내역 확인
    const historyRows = historyTable.locator('.q-table__middle tbody tr');
    const latestRow = historyRows.nth(0); // 최신 이력

    // then: 변경 내역 컬럼의 텍스트 가져오기
    const changedFieldsColumn = latestRow.locator('td').nth(1);
    const changedFieldsText = await changedFieldsColumn.textContent();

    // then: 메타 필드가 포함되지 않아야 함
    expect(changedFieldsText).not.toContain('수정 시각');
    expect(changedFieldsText).not.toContain('생성 시각');
    expect(changedFieldsText).not.toContain('수정자');
    expect(changedFieldsText).not.toContain('생성자');
    expect(changedFieldsText).not.toContain('updatedAt');
    expect(changedFieldsText).not.toContain('createdAt');
    expect(changedFieldsText).not.toContain('updatedBy');
    expect(changedFieldsText).not.toContain('createdBy');

    // then: 실제 변경된 필드(예약자명)가 표시되어야 함
    expect(changedFieldsText).toContain('예약자명');
  });

  test('테스트 데이터 정리', async ({ page }) => {
    test.skip(!createdReservationId, 'No reservation created in previous test');
